...
async function setupWithPatient(request) {
  ...
  const patientResp = await request.post(`${API_URL}/patients`, {
    ...
    data: { patient_name: `病理患者${RUN_ID}`, gender: 'female' },
  });
  if (patientResp.ok()) {
...
test.describe('病理报告 E2E', () => {
  ...
  test('病理报告页面加载', async ({ page }) => {
    await page.goto(`${BASE_URL}/home/pathology-reports`, { timeout: 15000, waitUntil: 'domcontentloaded' });
    await page.waitForURL(/\/home\/pathology-reports/, { timeout: 15000 });
  ...
  test('通过API创建病理报告', async ({ page, request }) => {
    const { token, patientId } = await setupWithPatient(request);
    if (!patientId) { test.skip(); return; }

    const pathoResp = await request.post(`${API_URL}/medical/pathology`, {
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      data: {
        patient_id: patientId,
        specimen_type: '组织活检',
        pathology_date: '2026-05-03',
        hospital: 'E2E病理医院',
        diagnosis: '未见恶性细胞',
      },
    });
