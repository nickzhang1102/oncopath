<template>
  <!-- Office文档预览组件 - 全屏iframe版本 -->
  <div v-if="visible" class="office-preview-overlay">
    <!-- 圆形关闭按钮 -->
    ...
    <div v-if="error" class="error-overlay">
      ...
      <p>Office文档预览失败</p>
      <p class="error-hint">{{ getFileTypeText() }}文件可能需要转换</p>
      <p class="error-detail" v-if="props.document">
        文档ID: {{ props.document.doc_id }}<br>
        文件名: {{ props.document.doc_name }}<br>
        文件类型: {{ props.document.file_type }}
      </p>
...
const buildPreviewUrl = () => {
  ...
  try {
    ...
    console.error('OfficePreview: 构建预览URL失败:', err)
    showToast(`Office预览URL构建失败: ${err.message}`)
    error.value = true
  } finally {
    loading.value = false
  }
...
const handleIframeError = () => {
  console.error('OfficePreview: iframe加载失败')
  loading.value = false
  error.value = true
  showToast('Office文档预览加载失败')
}
